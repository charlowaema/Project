...
$sql3="insert into receipts(No,Deposits,LoanRepaymentAmount,LoanType,Shares,ReceiptDate,PaymentMethod,LoanPenalty,Interest,AccruedInterest,LoanId,Transaction_Id)values('$memberno','0.00',0.00,'$loantype','0.00','$issuancedate','','','','',$loan_id','$transaction_id')";
...
$sql_20="Select sum(AccruedInterest) as AccruedInterestAmount from receipts INNER JOIN loans on loans.LoanId=receipts.LoanId where loans.No='$memberNo' AND loans.Status='Not Fully Paid' AND  loans.LoanType='$LoanType'   group by loans.LoanId";
$result20=mysqli_query($con,$sql_20);
$row20= mysqli_fetch_array($result20);


$sql_21="Select sum(AccruedInterestPayment) as AccruedInterestPayment from receipts INNER JOIN loans on loans.LoanId=receipts.LoanId where loans.No='$memberNo' AND loans.Status='Not Fully Paid' AND  loans.LoanType='$LoanType' AND AccruedInterestState='Payed'  group by loans.LoanId";
...
if($AmountDeposited==$precalculatedMonthlyInterest+$principle)
...
//old code
/*
if(($RepaymentAmount==0.00 || "")&&($Interest==0.00 || "")){
...
else if($RepaymentAmount>0.00 && ($RepaymentAmount>$_SESSION['AccruedInterestAmount']) && ($_SESSION['AccruedInterestAmount'])!=0.00){

    
      $Interest=$AccruedInterest;
      $AccruedInterest=0.00;
      $RepaymentAmount=$RepaymentAmount-($_SESSION['AccruedInterestAmount']);
      
      $AccruedInterestRepayment=$_SESSION['AccruedInterestAmount'];
      //$AccruedInterestName="Accrued Interest";
      $AccruedInterestState='Payed';
}

else if($RepaymentAmount>0.00 && ($RepaymentAmount<$_SESSION['AccruedInterestAmount'])){

     $Interest=0.00;
     $AccruedInterest=($_SESSION['AccruedInterestAmount']-$_SESSION['AccruedInterestPaymentAmount'])-$RepaymentAmount;
    
     $AccruedInterestRepayment=$RepaymentAmount;
      $RepaymentAmount=0.00;
      //kasee
      //$AccruedInterestName="Accrued Interest";
...
$sql="insert into receipts(No,Deposits,LoanRepaymentAmount,LoanType,Shares,ReceiptDate,PaymentMethod,LoanPenalty,Interest,AccruedInterest,AccruedInterestPayment,AccruedInterestState,LoanId,Transaction_Id)values('$Memberno','$Deposits','$RepaymentAmount','$LoanType','$Shares','$Receiptdate','$paymentmethod','$Loanpenalty','$Interest','$AccruedInterest','$AccruedInterestRepayment','$AccruedInterestState','$LoanId','$transaction_id')";
$result=mysqli_query($con,$sql);


//$sqlUpdate="update receipts set AccruedInterest=(AccruedInterest-$AccruedInterest) where LoanId='$LoanId' and No='$Memberno'";
//$resultUpdate=mysqli_query($con,$sqlUpdate);



//income 1(interest)
$sql22="insert into incomes(Income,ReceiptDate,Transaction_Id,IncomeType)values('$Interest','$todaysDate','$transaction_id','Interest')";
